...
//                panoramioLayer.clearMap();
  ...
  ['$window', '$location', '$rootScope', '$scope', 'UserPhoto', 'UserService', '$modal',
    ...
    function ($window, $location, $rootScope, $scope, UserPhoto, UserService, $modal,
      ...
      $scope.searchTypes = [{type:'photo', name: '图片'},
        {type: 'location', name: '地点'}]
      $scope.searchType = $scope.searchTypes[1];
      ...
      $scope.$on("searchChanged", function (e, val) {
        ...
        $scope.setSearchType($scope.searchTypes[0]);
      });
      ...
      $scope.getLocation = function (val) {
        ...
        if($scope.searchType.type=='location') {
          //if(angular.isString(val) && val.length<3) {
          //  d.resolve([]);
          //  return;
          //}
      ...
      $scope.goLocation = function (search) {
        if($scope.searchType.type=='photo') {
          ...
          if (location) {
            ...
            if(search.bounds) {
              $scope.mapEventListener.setBounds($scope.map, search.bounds.sw, search.bounds.ne);
            }
            $scope.searching = false;
          }else if(angular.isString(search)) {
            $scope.mapService.getLocPois(search).then(function (res) {
              if(res[0] && res[0].location) {
                $scope.goLocation(res[0]);
                $scope.searching = false;
              }
            });
          }
  ...
  .controller('MapsPopularCtrl',
  ['$window', '$location', '$rootScope', '$scope', 'UserPhoto', 'UserService', '$modal',
    'ponmCtxConfig', '$log', '$state', 'jsUtils',
    function ($window, $location, $rootScope, $scope, UserPhoto, UserService, $modal,
              ponmCtxConfig, $log, $state, jsUtils) {
      ...
      function getTravel(travelId) {
        TravelService.getTravel({travelId: travelId}, function (res) {
          if (res.status == "OK") {
            ...
            // 获取图片的用户信息
            UserService.getOpenInfo({'userId': res.travel.user_id}, function (data) {
              if (data.status == "OK") {
