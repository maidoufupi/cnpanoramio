    ...
    function (ponmCtxConfig, MessageService, alertService) {
      return {
        ...
        link: function (scope, element, attr) {
          ...
          var mediaWidth = 4, mediaHeight = 3;

          switch (scope.message.type) {
            case 'photo':
              scope.message.point = scope.message.photo.point;
              mediaWidth = scope.message.photo.width;
              mediaHeight = scope.message.photo.height;
              break;
            case 'travel':
              ...
              if (shareMesasage) {
                ...
                switch (shareMesasage.type) {
                  case 'photo':
                    ...
                    mediaWidth = shareMesasage.photo.width;
                    mediaHeight = shareMesasage.photo.height;
                    break;
                  case 'travel':
          ...
          var messageMedia = element.find(".message-media");
          if(messageMedia) {
            scope.mediaWidth = messageMedia.width();
            scope.mediaHeight = scope.mediaWidth * mediaHeight / mediaWidth;
          }
